...
class DatabaseService with ChangeNotifier {
  ...
  Future<String> uploadProfileImage(File image, String name) async {
    fbStore.Reference ref =
        fbStore.FirebaseStorage.instance.ref('userImages/$name');
  ...
  Future<void> updateUserInfo(
      String email, String fullName, String bDate, String bio) async {
    String uid = FirebaseAuth.instance.currentUser.uid.toString();
    await userCollection.doc(uid).update(
        {'email': email, 'fullName': fullName, 'bDate': bDate, 'bio': bio});
  }
  ...
  Future<void> addNewUserAdditionalData() async {
    ...
    await userCollection
        .doc(uid)
        .update({'lastSeen': [], 'messages': [], 'likes': [], 'matches': []});
  }
  ...
  Future<PaymentModel> getMyPaymentInfo() async {
    String _uid = FirebaseAuth.instance.currentUser.uid.toString();
    var result =
        await firestoreInstance.collection('paymentRequests').doc(_uid).get();
  ...
  Future<void> sendNewRequestEmail(PaymentModel paymentModel) async {
    ...
    Map<String, String> banks = {
      "CBE": "1000145305717",
      "Hibret Bank": "1000145305717",
      "Dashen Bank": "1000145305717",
      "Wegagen Bank": "1000145305717"
    };
  ...
  Future<void> updatePayment(
      {String transactionRef, String depositedByName}) async {
    String _uid = FirebaseAuth.instance.currentUser.uid.toString();
    await paymentCollection.doc(_uid).update({
  ...
  Future<String> uploadAudio(File audio, String name) async {
    fbStore.Reference ref =
        fbStore.FirebaseStorage.instance.ref('userAudio/$name');
  ...
  Future<void> getNewDates(BuildContext context) async {
    ...
    var result = await firestoreInstance
        .collection('users')
        .where('gender', isEqualTo: me.interest)
        .get();
  ...
  Future<bool> updateOnDisLikes(String otherUid) async {
    ...
    LikesModel lastSeenModel = LikesModel(
        type: 0, like: refOther.path.toString(), date: DateTime.now());

    //update my lastSeen list
    ref.update({
      'lastSeen': FieldValue.arrayUnion([lastSeenModel.toJson()])
    });
    return true;
  }
  ...
  Future<bool> updateOnLikes(int type, String otherUid) async {
    ...
    LikesModel like2 = LikesModel(
        type: type, like: refOther.path.toString(), date: DateTime.now());
    LikesModel like =
        LikesModel(type: type, like: ref.path.toString(), date: DateTime.now());

    //update my lastSeen list
    ref.update({
      'lastSeen': FieldValue.arrayUnion([like2.toJson()])
    });
    ...
    MessagingService.sendToTopic(
        title: 'Likes', body: 'Someone liked you', topic: '${otherUid}_like');
  ...
  Future<bool> updateMatch(UserModel me, UserModel other) async {
    ...
    meRef.update({
      'matches': FieldValue.arrayUnion([otherRef.path.toString()])
    });
    otherRef.update({
      'matches': FieldValue.arrayUnion([meRef.path.toString()])
    });

    //Send New Match notification
    MessagingService.sendToTopic(
        title: 'You have a new match 🥂',
        body: 'click to see who your match is',
        topic: '${me.uid}_match');
  ...
  Future<bool> removeMatches(String otherReference) async {
    ...
    final String myRef =
        firestoreInstance.collection('users').doc(uid).path.toString();
    ...
    try {
      await firestoreInstance.doc(myRef).update({
        'matches': FieldValue.arrayRemove([otherReference])
      });
    } catch (e) {
  ...
  Future<bool> createConversation(UserModel me, UserModel other) async {
    ...
    ConversationModel convoModel = new ConversationModel(
      ...
      lastMessage: LastMessageModel(
          message: 'No Chats', from: 'both', read: true, time: DateTime.now()),
      messageCount: 0,
      unreadMessage: 0,
    ...
    var convoRef =
        firestoreInstance.collection('conversations').doc('$convoId');

    //Update user messageList (both)
    await wRef.update({
      'messages': FieldValue.arrayUnion([convoRef.path.toString()])
    });
    await mRef.update({
      'messages': FieldValue.arrayUnion([convoRef.path.toString()])
    });
    ...
    MessagingService.sendToTopic(
        ...
        body: 'You can now chat with ${other.fullName}',
        topic: '${me.uid}_convo');
  ...
  Future<void> sendMessage(
      String message, String replyTo, String type, String convoRef) async {
    UserModel me = await getMyInfo();

    DateTime date = new DateTime.now();

    //Create the Message model
    final MessageModel _messageModel = new MessageModel(
        message: message,
        from: me.uid,
        replyTo: replyTo,
        time: date,
        messageId: date.toIso8601String(),
        type: type);
    ...
    try {
      await firestoreInstance
          ...
          .set(_messageModel.toJson());
    } catch (e) {
      print('not creating the new collection');
    }

    //update the last message, message count, unread messages and last Update
    final LastMessageModel _lastMessage = new LastMessageModel(
        message: message, from: me.uid, read: false, time: date);
    await firestoreInstance.doc(convoRef).update({
    ...
    MessagingService.sendToTopic(
        title: me.fullName, body: message, topic: convoRef.split('/')[1]);
  }
  ...
  Future<void> deleteConversation(String convoRef) async {
    ...
    await firestoreInstance.doc(user1Ref).update({
      'messages': FieldValue.arrayRemove([convoRef])
    });
    await firestoreInstance.doc(user2Ref).update({
      'messages': FieldValue.arrayRemove([convoRef])
    });
