...
class _HomeState extends State<Home> {
  ...
  Widget build(BuildContext context) {
    return loading
        ...
        : (Provider.of<DatesState>(context).getMyDates.length ==
                        Provider.of<MainHomeState>(context).getPage ||
                    Provider.of<DatesState>(context).getMyDates.length == 0) &&
                !Provider.of<MatchState>(context).getMatchState
            ? Container(
            ...
            : Container(
                ...
                child: Provider.of<MatchState>(context).getMatchState
                    ? MatchView(
                        otherUser:
                            Provider.of<DatesState>(context).getMyDates[_page],
                        me: me)
                    : PageView.builder(
                        itemCount:
                            Provider.of<DatesState>(context).getMyDates.length,
...
//          }
                          ...
                          return SingleUserView(
                            userModel: Provider.of<DatesState>(context)
                                .getMyDates[_page],
                            fromHome: true,
                            changeThePage: index !=
                                    Provider.of<DatesState>(context)
                                            .getMyDates
                                            .length -
                                        1
                                ? changeCurrentPage
                                : null,
                            pageBack: index == 0 ? null : pageBack,
                          );
                        },
                      ),
              );
