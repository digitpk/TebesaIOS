...
class SingleUserView extends StatefulWidget {
  ...
  SingleUserView(
      {this.userModel, this.fromHome, this.changeThePage, this.pageBack});
...
class _SingleUserViewState extends State<SingleUserView> {
  ...
  int _ageFromString(String dateString) {
    ...
    DateTime date = new DateTime(
        int.parse(list[2]), int.parse(list[1]), int.parse(list[0]));
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
        stream: FirebaseFirestore.instance
            .doc('users/${FirebaseAuth.instance.currentUser.uid.toString()}')
            .snapshots(),
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        image: DecorationImage(
                          image: NetworkImage(
                              widget.userModel.photos[selectedPic]),
                          fit: BoxFit.cover,
                        ),
                      ),
                    ),
                        ...
                        children: [
                          ...
                          (widget.pageBack == null)
                              ...
                              : widget.fromHome
                                  ...
                                  : Container(),
                          (widget.fromHome && widget.changeThePage != null)
                              ? IconButton(
                                  icon: Icon(
                    ...
                    _checkIfLastSeen(widget.userModel.uid, _lastSeen)
                        ? Container(
                            child: Expanded(
                              child: Text(
                                Lang.language == 0
                                    ? 'You have reacted to this user before.'
                                    : 'ከዚህ በፊት ለዚህ ተጠቃሚ ምላሽ ሰጥተዋል',
                                style: TextStyle(
                                    fontSize: 17.0,
                        ...
                        : Row(
                            ...
                            children: [
                              SpringButton(
                                ...
                                onTap: () async {
                                  if (_checkIfLastSeen(
                                      widget.userModel.uid, _lastSeen)) {
  ...
  Future<void> changePage(int from, bool next, BuildContext context) async {
    if (from == 1) {
      ...
      bool res =
          await _databaseService.updateOnLikes(from, widget.userModel.uid);
    }

    bool checkIfMatch =
        await _databaseService.checkIfMatch(widget.userModel.uid);
